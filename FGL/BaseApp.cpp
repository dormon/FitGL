...
BaseApp::Options BaseApp::options;
...
SDLWindowS BaseApp::addWindow(SDLWindowS const & window) {
...
SDLWindowS BaseApp::addWindow(int width, int height, bool resizable, bool fullscreen, int multisample) {
...
void BaseApp::addDrawCallback(Callback const & callback, SDLWindowS window) {
	if (window == nullptr)window = mainWindow;
	DrawCallbackFilter dc = { callback, SDLWindowW(window) };
